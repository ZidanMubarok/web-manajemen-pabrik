...
<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3 bg-primary text-white">
            <h6 class="m-0 font-weight-bold"><i class="fas fa-industry me-2"></i> Ajukan Order Baru ke Pabrik</h6>
        </div>
        <div class="card-body">
            ...
            <?php if (empty($availableProducts)): ?>
                <div class="alert alert-warning text-center" role="alert">
                    Belum ada produk yang tersedia dari Pabrik.
                </div>
            <?php else: ?>
                <form action="<?= base_url('distributor/orders/store-to-pabrik') ?>" method="post">
                    <?= csrf_field() ?>
                            ...
                            <tbody>
                                ...
                                foreach ($availableProducts as $product): ?>
                                    <tr>
                                        ...
                                        <td>Rp <span class="unit-price" data-price="<?= esc($product['base_price']) ?>"><?= esc(number_format($product['base_price'], 0, ',', '.')) ?></span></td>
                                        <td>
                                            <input type="number" name="product_quantities[<?= esc($product['id']) ?>]"
                                                class="form-control quantity-input" min="0" value="0"
                                                data-product-id="<?= esc($product['id']) ?>"
                                                style="width: 80px;">
                                        </td>
                                        <td>Rp <span class="sub-total" id="sub_total_<?= esc($product['id']) ?>">0</span></td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
                            <tfoot>
                    ...
                    <button type="submit" class="btn btn-primary mt-3"><i class="fas fa-paper-plane me-1"></i> Ajukan Order ke Pabrik</button>
                    <a href="<?= base_url('distributor') ?>" class="btn btn-secondary mt-3 ms-2"><i class="fas fa-times me-1"></i> Batal</a>
                </form>
            <?php endif; ?>
        </div>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        quantityInputs.forEach(input => {
            ...
            input.dispatchEvent(new Event('input')); // Trigger on load for initial calculation
        });
